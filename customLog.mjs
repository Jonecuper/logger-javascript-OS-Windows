...
function logSync(message, level) {
  ...
  if (fs.existsSync(logFile) && fs.statSync(logFile).size > config.maxFileSize) {
    const backupFile = path.join(logPath, `${path.parse(config.logFileName).name}_${new Date().toISOString().replace(/:/g, '-')}.txt`);
...
/**
 * Основная функция логгера. Работает в синхронном или асинхронном режиме.
 *
 * @param {string} message Сообщение для логирования.
 * @param {string} level Уровень логирования (INFO, ERROR, DEBUG).
 ...
 */
export function customLog(message, level = 'INFO', isAsync = false) {
  if (isAsync) {
